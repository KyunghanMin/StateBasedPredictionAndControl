...
def model_det(model_name):
    ...
    ModDens1 = Dense(1, activation='tanh',input_shape=(ModelConfig_NumInputSequence,ModelConfig_NumLstmUnit))
    model.add(ModDens1)
    ModReshape1 = Reshape((ModelConfig_NumInputSequence,))
    model.add(ModReshape1)
    ModDens2 = Dense(1, activation='tanh',input_shape=(1,ModelConfig_NumInputSequence))
    model.add(ModDens2)
...
#%% Model configuration
ModelConfig_NumInputSequence = 12;
ModelConfig_NumFeature = 4;
ModelConfig_NumLstmUnit = 14;
ModelConfig_NumOutputSize = 1;
ModelConfig_NumEpoch = 30;
ModelConfig_NumBatch = 10;
...
#model_All.save('RnnBraking_OptC_all')
##%% Fit network_c1
#K.clear_session()
#model_det('model_c1')    
...
#model_c1.save('RnnBraking_OptC_c1')
##%% Fit network_c2
#K.clear_session()  
#model_det('model_c2')    
...
#model_c2.save('RnnBraking_OptC_c2')
##%% Fit network_c3  
#K.clear_session()
#model_det('model_c3')      
...
#model_c3.save('RnnBraking_OptC_c3') 
#%% Model validation for test case - Load model data    
#ValidKeyList = random.sample(DataLoad.keys(),50)
DicPreResult = {'Predict_Int':[],'Predict_Clu':[],'Driver':[],'Case':[],'MSE_Int':[],'MSE_Clu':[]}

#model_All = load_model('RnnBraking_4dim_SimpleRNN_TwoMLP')
model_All = load_model('RnnBraking_OptC_all')
model_C1 = load_model('RnnBraking_OptC_C1')
model_C2 = load_model('RnnBraking_OptC_C2')
model_C3 = load_model('RnnBraking_OptC_C3')

#%% Determine intermediate model
layer_name = 'dense_1'
model_dense1 = Model(inputs=model_All.input,
                                 outputs=model_All.get_layer(layer_name).output)
print(model_dense1.summary())
layer_name_rnn = 'lstm_1'
model_seq = Model(inputs=model_All.input,
                                 outputs=model_All.get_layer(layer_name_rnn).output)
print(model_seq.summary())
#%% Validation - prediction

for i in IndexKeySet:    
#    i = ValidKeyList[1];
    ...
    PredictArry_State  = np.zeros([PredictionRange,ModelConfig_NumLstmUnit])
    PredictArry_Sequence  = np.zeros([PredictionRange,ModelConfig_NumInputSequence])
    ...
    for j in range(PredictionRange):
        ...
        Predict_Value_Seq = model_seq.predict(ValidDataSet_X)
        Predict_Value_Dense1 = model_dense1.predict(ValidDataSet_X)
        PredictArry_State[j,:] = np.reshape(Predict_Value_Seq[0,-1,:],(1,ModelConfig_NumLstmUnit))
        PredictArry_Sequence[j,:] = np.reshape(Predict_Value_Dense1,(1,ModelConfig_NumInputSequence))
    ...
    # Save result
    tmpAccRef = ValidDataSet_Y[ModelConfig_NumInputSequence:]
    tmpAccEst = PredictArry[:,0]
    tmpAccErr = mean_squared_error(tmpAccRef,tmpAccEst)
    PredictData = [PredictArry,PredictArry_State,PredictArry_Sequence,DataLoad[i]]    
    DicPreResult['Predict_Int'].append(PredictData)
    DicPreResult['MSE_Int'].append(tmpAccErr)
    ...
    tmpAccEst = PredictArry[:,0]
    tmpAccErr = mean_squared_error(tmpAccRef,tmpAccEst)
    DicPreResult['MSE_Clu'].append(tmpAccErr)

import pickle
    
with open('PredictResult.pickle','wb') as mysavedata:
     pickle.dump(DicPreResult,mysavedata)     
mysavedata.close()    
#%% Plot prediction result
Index_cl1 = np.array(DicPreResult['Case'])==1
tmpY1_cl = np.array(DicPreResult['MSE_Clu'])[Index_cl1]
tmpY1_int = np.array(DicPreResult['MSE_Int'])[Index_cl1]

Index_cl2 = np.array(DicPreResult['Case'])==2
tmpY2_cl = np.array(DicPreResult['MSE_Clu'])[Index_cl2]
tmpY2_int = np.array(DicPreResult['MSE_Int'])[Index_cl2]

Index_cl3 = np.array(DicPreResult['Case'])==3
tmpY3_cl = np.array(DicPreResult['MSE_Clu'])[Index_cl3]
tmpY3_int = np.array(DicPreResult['MSE_Int'])[Index_cl3]

figure, axe = plt.subplots(3,1)
axe[0].plot(tmpY1_cl,'o')
axe[0].plot(tmpY1_int,'^')

axe[1].plot(tmpY2_cl,'o')
axe[1].plot(tmpY2_int,'^')
    
axe[2].plot(tmpY3_cl,'o')
axe[2].plot(tmpY3_int,'^')
#%% Plot one case of prediction
Index_Plot = random.sample(IndexKeySet)
